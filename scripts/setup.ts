...
(async () => {
	try {
		await replace({
			...
			from: /github.com\/ptkdev-boilerplate\/node-telegram-bot-boilerplate/g,
	...
	try {
		await replace({
			...
			from: /github.com\/ptkdev\/node-telegram-bot-boilerplate/g,
			to: setup.github_full_repository_url,
		});
	} catch (error) {
		console.error("Error occurred:", error);
	...
	try {
		await replace({
			...
			to: setup.github_repository_url.replace(
				"github.com",
	...
	try {
		await replace({
			...
			from: /@ptkdev\/node-telegram-bot-boilerplate/g,
			to:
				setup.package_org !== ""
					? `${setup.package_org}/${setup.package_name}`
					: setup.package_name,
		});
	} catch (error) {
		console.error("Error occurred:", error);
	...
	try {
		await replace({
			...
			from: /"projectName": "ptkdev-boilerplate\/node-telegram-bot-boilerplate"/g,
			to: `"projectName": "${setup.github_full_repository_url.replace(
				"github.com/",
				"",
			)}"`,
		});
